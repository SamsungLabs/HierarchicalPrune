...
"""Fine-tuning script for diffusion models (without knowledge distillation).

Standard diffusion fine-tuning using flow matching loss.
Supports DC-AE VAE replacement and pruned transformer architectures.
"""
...
from model.builder import get_vae, vae_encode
...
These are {repo_id} finetuned weights for {base_model}.
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(5) when distilling with mixture of different KD_feat types, use SA,CA,LFImg,LFCond"
        ),
    )
    parser.add_argument(
        "--caption_type",
...
def main(args):
    ...
    # Load VAE
    vae = get_vae(args.vae_name, args.vae_pretrained, accelerator.device, args.cache_dir)
    ...
    # If Needed, we disable some transformer blocks from being updated.
    cut_blocks_l = parse_cut_blocks(args.cut_transformer_blocks)
    for i in cut_blocks_l:
        transformer.transformer_blocks[i].requires_grad_(False)
    print(f"[INFO][Transformer] Disable {cut_blocks_l} transformer blocks from being updated")
    print(
