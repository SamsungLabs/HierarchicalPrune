...
VAE_NAME="sd3.5"
VAE_PRETRAINED="stabilityai/stable-diffusion-3.5-large-turbo"
MODEL_PRETRAINED="stabilityai/stable-diffusion-3.5-large-turbo"
...
CUT_BLOCKS="25-29,31-35" # Transformer blocks that are removed or pruned.
...
OUTPUT_DIR="../distil_Diffusers_ckpts/outputs/"$DATE"_sd35-large-turbo-distil"$KD_FEAT_TYPE"_"$CUT_TX_TYPE"_scaleRange"$SCALING_RANGE"_no_kd_task_lambda_kd_feat"$LAMBDA_KD_FEAT"_kd_loss_scaling"$KD_LOSS_SCALING"_res"$RESOLUTION"_bcz$((BATCH_SIZE*NUM_GPUS*GRAD_ACCUMULATION))_lr"$LR"_cut"$CUT_BLOCKS"_cut2"$CUT_BLOCKS_2"_rate"$CUT_RATIOS"_TrStep"$MAX_TRAIN_STEPS

echo $OUTPUT_DIR

StartTime=$(date +%s)
accelerate launch --config_file=configs/deepspeed_config.yaml --main_process_port=26502 \
