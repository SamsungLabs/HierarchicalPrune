...
class DCAE(nn.Module):
    ...
    def encode(self, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        x = self.encoder(x)
        # **kwargs is added to make it compatible with diffusers pipeline implementation
        # where it gives return_dict=False as additional argument to vae.encode
        if not kwargs.get('return_dict', True):
            return (x,)
        return x

    def decode(self, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        x = self.decoder(x)
        # **kwargs is added to make it compatible with diffusers pipeline implementation
        # where it gives return_dict=False as additional argument to vae.decode
        if not kwargs.get('return_dict', True):
            return (x,)
        return x
