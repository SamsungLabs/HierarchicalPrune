"""Contribution analysis codebase for transformer block importance.

Evaluates the impact of removing individual transformer blocks (or sub-components) on image generation quality.
Results are used to guide hierarchical pruning decisions.
"""
...
from model.builder import load_distilled_pipeline
from utils.utils import get_num_transformer_blocks, get_transformer_block_components, return_none
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The number of images per prompt for inference.",
    )
    parser.add_argument(
        "--validation_prompt",
        type=return_none,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=("Perform fine-grained contribution analysis"),
    )
    parser.add_argument(
        "--fine_grained_cont_analysis_type",
        type=return_none,
        default=None,
        help=("Types of fine-grained contribution analysis."),
    )
    parser.add_argument(
        "--debug",
        action="store_true",
...
def main(args):
    ...
    num_transformer_blocks, num_transformer_blocks_wo_single_blocks = get_num_transformer_blocks(
        pretrained_name=args.pretrained_model_name_or_path, cache_dir=args.cache_dir
    )
    take = int(args.benchmark_take) if args.benchmark_take is not None else None

    for tx_block_idx in range(num_transformer_blocks):
        tx_block_components = get_transformer_block_components(
            pretrained_name=args.pretrained_model_name_or_path,
            fine_grained_cont_analysis=args.fine_grained_cont_analysis,
            fine_grained_cont_analysis_type=args.fine_grained_cont_analysis_type,
            tx_block_idx=tx_block_idx,
            num_transformer_blocks_wo_single_blocks=num_transformer_blocks_wo_single_blocks,
        )
        for tx_block_component in tx_block_components:
            # Load Pipeline
            pipe, cut_blocks_l, n_params_tx_orig, n_params_tx_cut = load_distilled_pipeline(
                ...
                cut_transformer_blocks=str(tx_block_idx),
                cut_transformer_blocks_component=tx_block_component,
            ...
            full_output_dir_path = Path(args.output_dir).joinpath(
                ...
                f"cut_blk_{tx_block_idx}_{tx_block_component}"
                if args.fine_grained_cont_analysis
                else f"cut_blk_{tx_block_idx}",
            )
            config_gen_img = {
                "args": vars(args),
                "transformer_pretrained": args.transformer_pretrained,
                ...
                "num_tx_block_components": len(tx_block_components),
                "num_cont_analysis": num_transformer_blocks * len(tx_block_components),
                "cut_blocks_l": cut_blocks_l,
                "cut_blocks_component": tx_block_component,
            ...
            if not full_output_dir_path.joinpath(
                ...
                if args.debug:
                    print(
                        f"[INFO] Saving a config file to {full_output_dir_path.joinpath('config.json')}"
                    )
            ...
            if args.benchmark_type.lower() in ["hps", "hpsv2"]:
                ...
                if not done_already:
                    ...
                    for model_id, data in score.items():
                        for style, res in data.items():
                            ...
                            config_gen_img[args.benchmark_type][style][
                                "Avg"
                            ] = f"{np.mean(avg_score):.5f}"
                            config_gen_img[args.benchmark_type][style][
                                "Std"
                            ] = f"{np.std(avg_score):.5f}"
                ...
                NUM_IMAGES_PER_PROMPT = (
                    getattr(args, "num_images_per_prompt")
                    if getattr(args, "num_images_per_prompt", None) is not None
                    else 4
                )
                ...
                if not done_already:
                    ...
                    print(
                        f"[INFO] % correct prompts: {df.groupby('metadata')['correct'].any().mean():.2%}"
                    )
                    print()
                ...
                PIPE_BATCH_SIZE = min(NUM_IMAGES_PER_PROMPT, 2)
                with open(metadata_path) as f:
                ...
                for index, prompt_id in enumerate(prompt_with_id_list):
                    ...
                    for i in range(PIPE_BATCH_SIZE):
                        ...
                        for img in images:
                            img.save(
                                full_output_dir_path.joinpath("imgs", f"{id}_{sample_count}.png")
                            )
                            sample_count += 1
                    progress_bar.update(1)
                    progress_bar.set_postfix_str(
                ...
                if not done_already:
                    ...
                    for prompt_id in tqdm(
                        prompt_with_id_list,
                        desc=f"ImageReward {NUM_IMAGES_PER_PROMPT} images / prompt",
                    ):
                        id = prompt_id["id"]
                        prompt = prompt_id["prompt"]
                ...
                raise ValueError(
                    f"Invalid argument: args.benchmark_type=={args.benchmark_type} is given."
                )
