...
"""Distillation training codebase for SD3/3.5 transformer models.

Trains a pruned (student) SD3/3.5 transformer via knowledge distillation from
the original (teacher) model. Supports three loss components:
...
def main(args):
    ...
    ##### HPP (Hierarchical Position Pruning) #####
    # Replaces specified later blocks with identity blocks, keeping early blocks intact.
    # See paper Sec. 3.2 for details on the inter-block hierarchy.
    (
        transformer,
        cut_blocks_l,
        cut_fg_dict,
    ...
    ##### PWP (Positional Weight Preservation) + SGDistill (Sensitivity-Guided Distillation) #####
    # PWP: Freezes all early blocks (before the first pruned block) to preserve pretrained
    #       weights essential for image formation.
    # SGDistill: Implemented as a binary form of inverse distillation weighting: frozen
    #       (sensitive) blocks receive zero gradient updates, while remaining blocks receive
    #       full updates. This concentrates learning on less sensitive components.
    ...
    if vars(args).get("kd_loss_scaling", None):
        ...
        transformer.requires_grad_(False)  # PWP: freeze everything first
        grad = False  #  grad flips to True (i) when block indices are in kd_loss_scaling_range or (ii) after the first identity (pruned) block
        if vars(args).get("kd_loss_scaling_range", None):
            ...
            for i, (n, m) in enumerate(transformer.named_modules()):
                if n.startswith("transformer_blocks.") and grad == True:
                    idx = int(n.split(".")[1])
    ...
    if args.distill_type == "pixel":
        ...
        # load vae_teacher
        vae_teacher = get_vae(
            "sd3.5", "stabilityai/stable-diffusion-3.5-medium", accelerator.device, args.cache_dir
        )
    ...
    pipeline_text_data = StableDiffusion3Pipeline.from_pretrained(
        args.pretrained_model_name_or_path,
    ...
    if args.torch_compile:
        ...
        pipeline_text_data.text_encoder_3 = torch.compile(pipeline_text_data.text_encoder_3)
    ...
    if args.lambda_kd_feat > 0.0:
        ...
        if is_compiled_module(transformer_teacher):
            filter_str = "_orig_mod.transformer_blocks."
            module_name_length = 3
        else:
            filter_str = "transformer_blocks."
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                with torch.no_grad():
                    ...
                    model_pred_teacher = transformer_teacher(
                        hidden_states=noisy_model_input_teacher,
                        timestep=timesteps,
                        encoder_hidden_states=prompt_embeds,
                        pooled_projections=pooled_prompt_embeds,
                        return_dict=False,
                    ...
                    if args.distill_type == "pixel":
                        model_pred_teacher = vae_decode(
                            "sd3.5", vae_teacher, model_pred_teacher.to(weight_dtype)
                        ).to(dtype=weight_dtype)
                ...
                if accelerator.is_main_process:
                    ...
                    if validation_prompts is not None and global_step % args.validation_steps == 0:
                        ...
                        if args.vae_name == "dc-ae":
                            pipeline = DCAEStableDiffusion3Pipeline.from_pretrained(
                                args.pretrained_model_name_or_path,
                                vae=None,
